...
public interface CustomerController {
	...
	public ResponseEntity<CustomerResponse> createCustomerUsingPOST(
			@ApiParam(value = "CustomerReqeust", required = true) @Valid @RequestBody CustomerCreateRequest CustomerReqeust)
			throws Exception;
	...
	@RequestMapping(value = "/v1.0/customers/{id}", produces = {
	...
	@RequestMapping(value = "/v1.0/customers", produces = {
			"application/json" }, consumes = {}, method = RequestMethod.GET)
	public ResponseEntity<Page<Customer>> retrieveAllCustomerUsingGET(
			@PageableDefault(page = 0, size = 10) Pageable pageable) throws Exception;

	@ApiResponses(value = { @ApiResponse(code = 201, message = "Created"),
			@ApiResponse(code = 401, message = "Unauthorized"), @ApiResponse(code = 403, message = "Forbidden"),
			@ApiResponse(code = 404, message = "Not Found"),
			@ApiResponse(code = 200, message = "OK", response = ModelMap.class) })
	@RequestMapping(value = "/v1.0/customers/{id}", produces = {
			"application/json" }, consumes = {}, method = RequestMethod.DELETE)
	public ResponseEntity<ModelMap> deleteCustomerUsingDELETE(
			@ApiParam(value = "id", required = true) @PathVariable(value = "id", required = true) String id)
			throws Exception;
	...
	public ResponseEntity<CustomerResponse> updateCustomerUsingPUT(
			@ApiParam(value = "customerUpdateRequest", required = true) @Valid @RequestBody CustomerUpdateRequest customerUpdateRequest,
			@ApiParam(value = "id", required = true) @PathVariable(value = "id", required = true) String id)
			throws Exception;
}
